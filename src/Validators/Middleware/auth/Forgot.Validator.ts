...
export const ForgotPassEmailVerificationValidator = (
    req: Request,
    res: Response,
    next: NextFunction
) => {
...
export const ForogotPassOTPVerification = (
    req: Request,
    res: Response,
    next: NextFunction
) => {
    const Verification_Cookie = req.cookies.EmailVerificationToken
    if (!Verification_Cookie) return res.status(Status_Codes.UNAUTHORIZED).json({
...
export const ForgotPassPassChangeValidator = ( 
    req : Request,
    res : Response,
    next : NextFunction
) => {
