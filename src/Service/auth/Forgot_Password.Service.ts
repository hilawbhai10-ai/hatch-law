...
export const EmailVerificationServiceLayer = async (
    Email: string
) => {
    ...
    if (!IsUserExsits.emailExists) throw new AppError("Email Not Found",
...
export const OtpVerificationServiceLayer = async (
    OTP: number,
    Cache_ID: string
): Promise<string> => {
    ...
    try {
        ...
        if (!Cached_payload) throw new AppError("Invlaid or Expired Cookie",
    ...
    if (Number(Cached_payload.OTP) !== OTP) throw new AppError("Invalid OTP",
        Status_Codes.BAD_REQUEST
    )
...
export const ResetPasswordServiceLayer = async (
    New_Password: string,
    Cache_ID: string,
) => {
    let IsChangingtrue;
    try {
        ...
        if (!IsChangingtrue) throw new AppError("Invalid or Expired Cookie",
            Status_Codes.UNAUTHORIZED
        )
    } catch (err) {
        if (err instanceof AppError && err.statusCode === Status_Codes.UNAUTHORIZED) throw new AppError(err.message, err.statusCode)
        throw new AppError("Internal Server Error",
            Status_Codes.INTERNAL_SERVER_ERROR)
    }
