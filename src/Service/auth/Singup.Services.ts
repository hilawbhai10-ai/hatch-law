...
import {
  ...
  generate9DigitNumber,
} from "../../Utils/Utils_func.js";
...
const USER_ONBOARDING_JWT_PASS =
  process.env.User_ONBOARDING_JWT_PASS || "HRHGFRGRYI";
const SALT_ROUNDS = process.env.SALT_ROUNDS || 5;
const PERM_TOKEN_PASSWORD1 =
  process.env.PERM_TOKEN_PASSWORD || "hrfdguvigruofgruofgr";

// const User = new User_class();

export const SignupServiceLayer = async (
  password: string,
  // DOB: string,
  username: string,
  email: string,
  ...
  const IsUserExists = await UserRepo.isUserExists(email, username);
    ...
    if (IsUserExistsErrs.length > 0) {
      ...
      throw new AppError(
        IsUserExistsErrs.map(
          (e) => `{Error Key : ${e.Error_key}, Error : ${e.Error}}`,
        ).join(", "),
        Status_Codes.Conflit,
      );
    }
    ...
    try {
      await Redis_Client.set(
        `auth:signup:email_verification:${String(Redis_ID)}`,
        JSON.stringify({
          username,
          email,
          // DOB,
    ...
    const Signup_verification_token = jwt.sign(
      { Temp_ID: Redis_ID },
      Temp_JWT_Pass,
      { expiresIn: "15m" },
    ); // need to set expiry
...
export const SignupVerificationLayer = async (
  OTP: number,
  Redis_Id: string,
) => {
  ...
  let rawUser;
  try {
    rawUser = await Redis_Client.getdel(
      `auth:signup:email_verification:${Redis_Id}`,
    ); // need to do getdel
  } catch (e) {
    throw new AppError(
      "Internal Server Error",
  ...
  try {
    await Redis_Client.set(
      `auth:signup:UserOnboarding:${NEWREDISID}`,
      JSON.stringify({
  ...
  const User_Onboarding_token = jwt.sign(
    { User_Onboarding_token: NEWREDISID },
    USER_ONBOARDING_JWT_PASS,
    { expiresIn: "15m" },
  ); // need to set expiry
  return User_Onboarding_token;
};
export const SignupUserOnbardingServiceLayer = async (
  Redis_ID: string,
  Questions: any,
): Promise<string> => {
  ...
  const perm_token = jwt.sign(
    { id: createdUser.Obj!.id },
    PERM_TOKEN_PASSWORD1,
    { expiresIn: "30d" },
  );
