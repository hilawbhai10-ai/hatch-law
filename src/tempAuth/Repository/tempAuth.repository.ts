...
export const findOrCreateUserByOAuth = async (profile: any) => {
  ...
  try {
    const account = await (Prisma_Instances as any).account.findUnique({
      where: {
    ...
    if (email) {
      const existingUser = await (Prisma_Instances as any).user.findUnique({
        where: { email },
      });

      if (existingUser) {
        // create account linked to existing user
        await (Prisma_Instances as any).account.create({
          data: {
            provider,
            providerAccountId,
    ...
    let username =
      email ? email.split('@')[0] : `${provider}_${providerAccountId.substring(0, 6)}`;
    ...
    let attempt = 0;
    while (
      await (Prisma_Instances as any).user.findUnique({ where: { username: candidate } })
    ) {
      attempt += 1;
    ...
    const newUser = await (Prisma_Instances as any).user.create({
      data: {
        email,
        username,
        accounts: {
...
export const createUserWithPassword = async (email: string, hashedPassword: string) => {
  ...
  let candidate = username;
  let attempt = 0;
  while (await (Prisma_Instances as any).user.findUnique({ where: { username: candidate } })) {
    attempt += 1;
    candidate = `${username}_${attempt}`;
  }
  username = candidate;
...
export const findUserByEmail = async (email: string) => {
  ...
  const user = await (Prisma_Instances as any).user.findUnique({ where: { email: emailLower } });
  return user || null;
};

export const findUserById = async (userId: string) => {
  console.log('Finding user by ID in DB:', userId);
  const user = await (Prisma_Instances as any).user.findUnique({ where: { id: userId } });
  return user || null;
};