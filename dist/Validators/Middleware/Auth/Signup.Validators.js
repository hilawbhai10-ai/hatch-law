...
export const Signup_Middleware = (req, res, next) => {
    const isSchemaValid = Signup_zod_schema.safeParse(req.body);
    if (!isSchemaValid.success)
        return res.status(Status_Codes.Invalid_Entity).json({ Error: isSchemaValid.error });
    next();
};
export const SignupVerificationMiddleware = (req, res, next) => {
    const Temp_Signup_Cookie = req.cookies.Temp_Signup_Session;
    if (!Temp_Signup_Cookie)
    ...
    try {
        ...
        return res.status(Status_Codes.UNAUTHORIZED).json({ Error: "Invalid or Expired Cookie" });
    }
...
export const SignupOnboardingQuestions = async (req, res, next) => {
    const User_Onboarding_Session = req.cookies.User_Onboarding_Session;
    if (!User_Onboarding_Session)
        return res.status(Status_Codes.UNAUTHORIZED).json({
    ...
    try {
        ...
        return res.status(Status_Codes.UNAUTHORIZED).json({ Error: "Internal Server Error" });
    }
    ...
    if (!req.body)
        req.body = {}; // law if you have time fix it 
