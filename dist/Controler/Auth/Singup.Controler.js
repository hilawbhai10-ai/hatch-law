import { SignupServiceLayer, SignupVerificationLayer, SignupUserOnbardingServiceLayer, } from "../../Service/auth/Singup.Services.js";
import Status_Code from "../../Constant/Status_Codes.js";
const Is_Secure = false;
export const SignupAuthControler = async (req, res) => {
...
export const SignupAuthVerificationControler = async (req, res) => {
    let User_Onboarding_token;
    try {
        User_Onboarding_token = await SignupVerificationLayer(Number(req.body.OTP), req.body.Redis_ID.Temp_ID);
    }
    catch (err) {
        console.log(err);
        if (err.statusCode === Status_Code.INTERNAL_SERVER_ERROR)
...
export const SignupUserOnbardingControlerLayer = async (req, res) => {
    let perm_token;
    try {
        perm_token = await SignupUserOnbardingServiceLayer(req.body.Redis_ID, req.body.Questions);
    }
    catch (err) {
        if (err.statusCode === Status_Code.INTERNAL_SERVER_ERROR)
            return res
...
//# sourceMappingURL=Singup.Controler.js.map